# Plan: Migrate to Controller-Runtime Client with Dual Implementation

 Overview

 Migrate from manual REST client to controller-runtime client with TWO parallel implementations for testing and comparison, similar to the status updater pattern. Both
 approaches will be switchable via environment variables with clear removal comments.
