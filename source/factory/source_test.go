...
func (suite *ByNamesTestSuite) TestAllInitialized() {
	...
	sources, err := ByNames(context.TODO(), &source.Config{Sources: ss}, mockClientGenerator)
...
func (suite *ByNamesTestSuite) TestOnlyFake() {
	...
	sources, err := ByNames(context.TODO(), &source.Config{Sources: []string{types.Fake}}, mockClientGenerator)
	suite.NoError(err, "should not generate errors")
...
func (suite *ByNamesTestSuite) TestSourceNotFound() {
	mockClientGenerator := new(MockClientGenerator)
	mockClientGenerator.On("KubeClient").Return(fakeKube.NewClientset(), nil)
	sources, err := ByNames(context.TODO(), &source.Config{Sources: []string{"foo"}}, mockClientGenerator)
	suite.Equal(err, source.ErrSourceNotFound, "should return source not found")
...
func (suite *ByNamesTestSuite) TestKubeClientFails() {
	...
	for _, name := range sourceUnderTest {
		_, err := ByNames(context.TODO(), &source.Config{Sources: []string{name}}, mockClientGenerator)
		suite.Error(err, name+" should return an error if kubernetes client cannot be created")
	}
}
...
func (suite *ByNamesTestSuite) TestIstioClientFails() {
	...
	for _, name := range sourcesDependentOnIstioClient {
		_, err := ByNames(context.TODO(), &source.Config{Sources: []string{name}}, mockClientGenerator)
		suite.Error(err, name+" should return an error if istio client cannot be created")
	}
}
...
func (suite *ByNamesTestSuite) TestDynamicKubernetesClientFails() {
	...
	for _, name := range sourcesDependentOnDynamicKubernetesClient {
		_, err := ByNames(context.TODO(), &source.Config{Sources: []string{name}}, mockClientGenerator)
		suite.Error(err, name+" should return an error if dynamic kubernetes client cannot be created")
	}
}
...
func TestBuild_InvalidSource(t *testing.T) {
	...
	src, err := Select(ctx, "not-a-source", p, &source.Config{LabelFilter: labels.NewSelector()})
	if src != nil {
