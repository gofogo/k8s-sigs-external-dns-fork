...
func newTestVirtualServiceSource(t *testing.T, loadBalancerList []fakeIngressGatewayService, ingressList []fakeIngress, gwList []fakeGatewayConfig) (*virtualServiceSource, error) {
	...
	for _, gw := range gwList {
		...
		_, err := fakeIstioClient.NetworkingV1beta1().Gateways(gw.namespace).Create(t.Context(), gwObj, metav1.CreateOptions{})
		if err != nil {
			return nil, err
		}
	}
...
func TestTransformerInIstioGatewayVirtualServiceSource(t *testing.T) {
	...
	t.Run("virtualservice strips managed fields", func(t *testing.T) {
		vsObj := &networkingv1beta1.VirtualService{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-vs",
				Namespace: "default",
				Labels:    map[string]string{"label1": "value1"},
		...
		_, err := istioClient.NetworkingV1beta1().VirtualServices(vsObj.Namespace).Create(t.Context(), vsObj, metav1.CreateOptions{})
		require.NoError(t, err)
