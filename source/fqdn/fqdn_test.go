...
func TestExecTemplate(t *testing.T) {
	tests := []struct {
		...
		// TODO; how to annotations
		{
			name: "annotations hostnames",
			tmpl: "{{.Name}}.example.com, {{.Name}}.example.org, {{.Labels.environment }}.example.com, {{ index .ObjectMeta.Annotations \"alb.ingress.kubernetes.io/scheme\" }}.{{ .Labels.environment }}.{{ index .ObjectMeta.Annotations \"dns.company.com/zone\" }}",
			obj: &testObject{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test",
					Namespace: "default",
		...
		// metadata.annotations["kubernetes.io/tls-acme"]
		// TODO: how to handle labels
		// TODO: if annotation not ext-dns, return something
		...
		// 	},
		// 	wantErr:  true,
		// 	errorMsg: "failed to apply template",
		// },
		// {
		// 	name:     "nil object",
...
// testObject implements kubeObject interface for testing
