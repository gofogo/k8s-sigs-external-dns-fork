...
# https://github.com/external-secrets/external-secrets/blob/main/hack/crd.generate.sh
set -euo pipefail
...
EXECUTABLE=$(which controller-gen)

go run ${EXECUTABLE} \
  object:headerFile="scripts/api/boilerplate.go.txt" \
  paths="./apis/..."
