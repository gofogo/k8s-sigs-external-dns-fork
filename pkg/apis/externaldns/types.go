...
func bindFlags(b FlagBinder, cfg *Config) {
	...
	b.RegexpVar("regex-domain-exclusion", "Regex filter that excludes domains and target zones matched by regex-domain-filter (optional); requires 'regex-domain-filter' and is ignored when it is unset", defaultConfig.RegexDomainExclusion, &cfg.RegexDomainExclusion)
